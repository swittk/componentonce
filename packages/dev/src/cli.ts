...
export async function runComponentOnceDevCli(args: readonly string[]): Promise<void> {
  if (args.length === 0 || args.includes("--help") || args.includes("-h")) {
    console.log("Usage: componentonce dev <entry.tsx> [--host browser-host.ts] [--external exact/specifier] [--port 4173] [--export definition]\n       componentonce-dev <entry.tsx> [same options]\nLocal trusted-code React workbench. No backend/production server or Next dependency.");
    return;
  }
  const entry = args[0];
  ...
  const server = await createComponentOnceDevServer({ entry, port, externalModules, definitionExport, ...(host === undefined ? {} : { host }) });
  console.log("ComponentOnce Dev  " + server.url + "\nWatching " + entry + "\nSource changes remount the component; fixtures stay in memory. Trusted local code only.");
